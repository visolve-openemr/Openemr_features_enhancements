...
function amcSoCProvided($amc_id, $patient_id, $object_category='', $object_id='0') {
	 sqlStatement("UPDATE `amc_misc_data` SET `soc_provided` = 1 WHERE `amc_id`=? AND `pid`=? AND `map_category`=? AND `map_id`=? AND `soc_provided` = 0", array($amc_id,$patient_id,$object_category,$object_id) );
}
// Function to set summary of care provided for a encounter/patient from the amc_misc_data sql table
//   $amc_id     - amc rule id
//   $patient_id - pid
...
function amcNoSoCProvided($amc_id, $patient_id, $object_category='', $object_id='0') {
	 sqlStatement("UPDATE `amc_misc_data` SET `soc_provided` = 0 WHERE `amc_id`=? AND `pid`=? AND `map_category`=? AND `map_id`=? AND `soc_provided` = 1", array($amc_id,$patient_id,$object_category,$object_id) );
}
?>
